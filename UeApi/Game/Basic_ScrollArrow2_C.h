#pragma once
/*
Blueprint class Basic_ScrollArrow2_C, generated by AssetGen/tools/genueapi.py. Do not edit.

A member is here if and only if AssetGen can compile a use of it.
*/
#include "../UeMeta.h"
#include "../CoreUObject.h"
#include "../Engine.h"
#include "../UMG.h"

class UHorizontalBox;
class UImage;
class USizeBox;

namespace Game { namespace UI { namespace Art { namespace WidgetParts {

class Basic_ScrollArrow2_C : public UUserWidget
{
public:
    UE_CLASS("/Game/UI/Art/WidgetParts/Basic_ScrollArrow2", "Basic_ScrollArrow2_C");
    FPointerToUberGraphFrame UberGraphFrame;
    class UImage* arrow_bracket;
    class UImage* arrow_main;
    class UHorizontalBox* HorizontalBox_102;
    class USizeBox* SizeBox_0;
    class USizeBox* SizeBox_1;
    TMulticastInlineDelegate<void()> OnClicked;
    float ButtonAngle;
    bool Active;
    void ExecuteUbergraph_Basic_ScrollArrow2(int EntryPoint);
    UE_COSMETIC void PreConstruct(bool IsDesignTime);
    FLinearColor SetActive(bool NewVar_0);
};

}}}}   // namespace Game::UI::Art::WidgetParts

using Basic_ScrollArrow2_C = Game::UI::Art::WidgetParts::Basic_ScrollArrow2_C;