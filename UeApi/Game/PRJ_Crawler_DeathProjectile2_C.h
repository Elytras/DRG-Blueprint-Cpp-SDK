#pragma once
/*
Blueprint class PRJ_Crawler_DeathProjectile2_C, generated by AssetGen/tools/genueapi.py. Do not edit.

A member is here if and only if AssetGen can compile a use of it.
*/
#include "../UeMeta.h"
#include "../CoreUObject.h"
#include "../Engine.h"
#include "../FSD.h"

class AGooGunPuddle;
class UAudioComponent;
class UDamageComponent;
class UNiagaraComponent;
class USoundBase;

namespace Game { namespace Enemies { namespace Crawler {

class PRJ_Crawler_DeathProjectile2_C : public AGooGunProjectile
{
public:
    UE_CLASS("/Game/Enemies/Crawler/PRJ_Crawler_DeathProjectile2", "PRJ_Crawler_DeathProjectile2_C");
    FPointerToUberGraphFrame UberGraphFrame;
    class UNiagaraComponent* P_SpiderLobber_Projecitle_Trail_TEMP_Converted;
    static constexpr const char* P_SpiderLobber_Projecitle_Trail_TEMP_Converted__UeScsNode = "a4fba5ec3c1aca4b83b9c1dd244eb239";
    class UNiagaraComponent* GooParticle;
    static constexpr const char* GooParticle__UeScsNode = "d1a08dd64475754ebf7e98d6205d9633";
    class UAudioComponent* Audio;
    static constexpr const char* Audio__UeScsNode = "fc032520d364cc4f8defd55e12860f19";
    class UDamageComponent* Damage;
    static constexpr const char* Damage__UeScsNode = "c87e3d363ceea447baae1298ac37533e";
    int MaxBounces;
    bool HasGravityOverclock;
    bool HasBuckShotOverclock;
    class AGooGunPuddle* SpawnedPuddle;
    bool ExplosivePuddles;
    class USoundBase* CreatureImpactSound;
    void ExecuteUbergraph_PRJ_Crawler_DeathProjectile2(int EntryPoint);
    void SpawnEffects(FVector Location, FVector Normal);
    void DestroyProjectile();
    void OnImpacted(bool PredictedImpact, FHitResult HitResult);
};

}}}   // namespace Game::Enemies::Crawler

using PRJ_Crawler_DeathProjectile2_C = Game::Enemies::Crawler::PRJ_Crawler_DeathProjectile2_C;